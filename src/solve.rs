...
pub fn balance(reagents: Vec<Substance>, products: Vec<Substance>) -> Vec<(String, i32)> {
    ...
    for (i, element) in all_atoms.iter().enumerate() {
        for rge in &reagents {
            subs.push(rge);
            let rxe = rge.atoms.get(element).map(|c| *c).unwrap_or(0 as u32).clone();
            matrix.push(rxe as f64);
        }
        for pde in &products {
            subs.push(pde);
            let rxe = pde.atoms.get(element).map(|c| *c).unwrap_or(0 as u32).clone();
            matrix.push(rxe as f64);
        }
    }
    ...
    let fc: Vec<Rational> = c.iter().map(|c| {
        limit_denominator(&Rational::from_f32(*c).unwrap().abs(), 100)
    }).collect();
    ...
    let mut fin: Vec<_> = fc.iter().map(|f| f * Rational::from((mult, 1))).map(|f| f.numer().to_i32().unwrap()).collect();
    fin.push(mult);
    let result: Vec<(String, i32)> = subs.iter().map(|s| s.to_owned().formula.clone()).zip(&mut fin.iter().map(|c| c.to_owned())).collect();
    result
...
mod tests {
    ...
    fn test() {
        ...
        let result: Vec<(&str, i32)> = bal.iter().map(|(s, c)| (s.as_str(), c.to_owned())).collect();
    ...
    // C6H5COOH + O2 = CO2 + H2O
    fn test_2() {
        let rg = vec![Substance::new("C6H5COOH", 3.0, None).unwrap(),
                      Substance::new("O2", 3.0, None).unwrap()];
        let pd = vec![Substance::new("CO2", 3.0, None).unwrap(),
    ...
    fn test_3() {
        let rg = vec![Substance::new("KMnO4", 3.0, None).unwrap(),
                      Substance::new("HCl", 3.0, None).unwrap()];
        let pd = vec![Substance::new("KCl", 3.0, None).unwrap(),
